...
;(function ( $, window, document, undefined ) {
    ...
    function onTabClick(){
        var _self = $(this); 
        var _idx,_mappingClassName;
        //alert($.data(this,"tabswitch-mapping-index"));
        if(_self.hasClass(actClassName)) return;
        _idx = $.data(this,"tabswitch-mapping-index");
        _self.siblings("."+actClassName).removeClass(actClassName);
        _self.addClass(actClassName);
        switchElement.removeClass(actClassName)
                    .filter(function(){
                        return $.data(this,"tabswitch-mapping-index")==_idx
                    })
                    .addClass(actClassName);
        if(callback)callback(_idx);
    }
    Plugin.prototype.init = function () {
       tabElement.bind(eventType,onTabClick);
    };

    // A really lightweight plugin wrapper around the constructor, 
    // preventing against multiple instantiations
    $.fn[pluginName] = function ( options ) {
        var _$target = $(options.target);
        return this.each(function (idx) {
            $(this).data("tabswitch-mapping-index",idx);
            _$target.eq(idx).data("tabswitch-mapping-index",idx);
...
$.extend($.validator, {
	...
	methods: {
		...
		birthday: function (value, element, param) {
		    return this.optional(element) || /^(\d{4})(-|\/)(([0-1]{1})([0-2]{1})|([0]{1})([0-9]{1}))(-|\/)(([0-2]{1})([0-9]{1})|([3]{1})([0-1]{1}))/.test(value);
		},

		taiwan_id: function (value, element, param) {
		    ...
		    i = tab.indexOf(firstchar);
		    if (i == -1) return false;
		    var sum = A1[i] + A2[i] * 9;

		    for (i = 1; i < 10; i++) {
		        v = parseInt(value.charAt(i), 10);
		...
		taiwan_tel: function (value, element) {
		    return this.optional(element) || /^\d{2,4}-\d{6,9}$/.test(value);
		},

		taiwan_mobile: function (value, element) {
		    return this.optional(element) || /^\d{2,4}-\d{6,9}$/.test(value);
		},
...
// 讀取視窗
// @params disply{boolean} -是否顯示
// @params msg {string}  -顯示訊息
function loading(disply, msg) {
    var rootPath, $mask;
    if (typeof jQuery === 'undefined') return;

    $mask = $('div.ui-overlay.loadmask');
    rootPath = $('body').data('root') || '/';

    if ($mask.length === 0) {
        $('<div/>', { 'class': 'ui-overlay loadmask', 'style': 'display:none;' }).append(
            $('<div/>', { 'class': 'ui-widget-overlay' }),
            $('<div/>', { 'class': 'ui-widget-shadow ui-corner-all' }),
            $('<div/>', { 'class': 'ui-widget ui-widget-content ui-corner-all' }).append(
                $('<div/>', { 'class': 'loading-image', alt: '' }),
                $('<span/>', { html: (msg || '資料處理中...') })
            )
        ).appendTo('body');
        $mask = $('div.ui-overlay.loadmask');
...
$(function() {
    ...
    $('.js-photoswipe-item').each(function(idx, val) {
        var $self = $(this);
        $self.data("index", idx);
        swipeItems.push(getSwipeItem($self));
    }).on('click', function(event) {
        event.preventDefault();
        options.index= $(this).data('index');
        new PhotoSwipe($pswp, PhotoSwipeUI_Default, swipeItems, options).init();
    });;

    function getSwipeItem(target) {
        var _item;
        var _href = target.attr('href'),
            _size = target.data('size').split('x'),
            _width = parseInt(_size[0], 10),
            _height = parseInt(_size[1], 10),
            _scale;
        if (_width > _height && _width >= MAX_WIDTH) {
            _scale = MAX_WIDTH / _width;
            _width *= _scale;
            _height *= _scale;
        }
        if (_height > _width && _height >= MAX_HEIGHT) {
            _scale = MAX_HEIGHT / _height;
            _width *= _scale;
            _height *= _scale;
        }
        _item = {
...
function getSiteToken() {
    var data = $('input[name="__RequestVerificationToken"]');
    return data.val();
}
...
$(function() {
    ...
    function refreshCaptcha() {
        $('#captcha-img').attr('src', "/Utility/Captcha?key=" + (new Date().getTime()));
        return false;
    }
...
$(function() {
    ...
    }, "格式：038000000或0919000000");

    $.validator.messages.required = '此欄位為必填';

    var $applyForm = $('#event-issue-report-form');

    $applyForm.on('keypress', function(e) {
    ...
    $('#btn-event-report').on('click', showReportBlk);
    $('.js-btn-hide-report').on('click', hideReportBlk);
    $('#event-report-blk').on('click', hideReportBlkByClickOverlay);

    $('.js-err-msg-adj').on({
        'focus': adjustErrorMsgPos,
        'blur': adjustErrorMsgPos
    });
    //驗證碼重整
    $('#btn-issue-refresh-captcha').on('click', refreshCaptcha);

    function refreshCaptcha() {
        $('#issue-captcha-img').attr('src', "/Utility/Captcha?key=" + (new Date().getTime()));
        return false;
    }
    
    function showReportBlk() {
        refreshCaptcha();
        $('#event-report-blk').addClass('show');
    }

    function hideReportBlk() {
        $('#captcha-img').attr('src', "/Utility/Captcha?key=" + (new Date().getTime()));
        $('#event-report-blk').removeClass('show');
        return false;
    }

    function hideReportBlkByClickOverlay(e) {
    ...
    function adjustErrorMsgPos(e) {
        if (e.type == "focus") {
            $applyForm.find('span.err').removeClass('adj');
            $(this).siblings('span.err').addClass('adj');
        } else {
            $(this).siblings('span.err').removeClass('adj');
        }
    }
        ...
        submitHandler: function(form) {
            $applyForm.ajaxSubmit({
                success: function(xhr) {
                    var _obj = JSON.parse(xhr);
                    if (_obj.success) {
                        $applyForm.find('input,textarea,select').val('');
                        hideReportBlk();
                        swal({
                            title: "訊息已送出",
                            text: "謝謝您的寶貴建議。",
...
$(function() {
    ...
    var currentSpecId,
        ...
        $body = $('body'),
        $cartNum = $('#cart-num'),
        $feeNormal = $('#fee-normal'),
        $feeRefrigeration = $('#fee-refrigeration'),
        $feeFrozen = $('#fee-frozen'),
        $feeOther = $('#fee-Other'),
        $shoppingFeeBlk = $('#shopping-fee-blk'),
        $fastBuySpecSelect = $('#fast-buy-spec-select'),
        $fastBuyNumSelect = $('#fast-buy-num-select'),
        $btnRemoveBucket = $('.js-remove-bucket');
        $bucketItemList = $('#bucket-item-list');
        $bucketItemsNum = $('#bucket-items-num');

    $('.js-add-cart').on('click', addItem2Cart);
    $('.js-remove-cart').on('click', removeConfirm);
    $('.js-shopping-item-num-ipt').on('change', updateShoppingItemNum);
    $('.js-shopping-item-num-add').on('click', adjShoppingItemNum);
    $('.js-shopping-item-num-minus').on('click', adjShoppingItemNum);
    $('.js-btn-buy-next-time').on('click', addToBuyNextTimeList);
    $fastBuySpecSelect.on('change', onSpecChange).val('0');
    $btnRemoveBucket.on('click', removeFromBucket)
    getCartInformation();

    function removeFromBucket() {
        var _bucketId = $(this).data('bucket-id');
        $.ajax({
                url: '/Shopping/DeleteFromBucketList',
                data: {
                    __RequestVerificationToken: getSiteToken(),
                    bucketID: _bucketId
                },
                type: 'POST',
                dataType: 'json'
            })
            .done(removeItemFromBucketList(_bucketId))
            .fail(ajaxError);
    }
    function removeItemFromBucketList(bucketId) {
        var _itemNum;
        $('#shopping-item-' + bucketId).slideUp(300, function() {
            $(this).remove();
            _itemNum = $bucketItemList.find('li').length;
            if(_itemNum!==0){
                $bucketItemsNum.html(_itemNum);
            }else{
                window.location.reload();
            }            
        });
    }
    function addToBuyNextTimeList() {
        var _self = $(this),
            _specId, _productId;
        var _specId;
        if ($body.data('is-login') !== 'True') {
            swal({
                title: "下次再買為會員功能，請先登入會員。",
                text: "",
                type: "info",
                ...
                document.location.href = '/Member/SignIn';
            });
            return;
        }
        _productId = _self.data('product-id');
        _specId = $fastBuySpecSelect.val() || _self.data('product-spec');
        if (_specId === '0') {
            swal({
                title: '請先選擇規格',
                text: "",
        ...
        $.ajax({
            ...
            .done(addToBucketListSuccess)
            .fail(ajaxError);
    }

    function addToBucketListSuccess(resp) {
        if (resp.success) {
            swal({
                title: "商品已成功加入下次購買清單。",
                text: "",
    ...
    function onSpecChange() {
        var _self = $(this);
        var _specId = _self.val();
        if (_specId === '0') {
            $('.js-fast-buy-func-blk').show();
            $('#blk-btn-notify,#no-stock-hint').addClass('hide');
            $('#no-stock-hint').removeClass('shake');
            return;
        }
        checkTheSpecStock(_specId);
    }

    function checkTheSpecStock(specId) {
        $.ajax({
                url: '/Shopping/SpecificationResidual',
                type: 'POST',
                dataType: 'json',
                data: {
                    __RequestVerificationToken: getSiteToken(),
            ...
            .done(setTheSpecStock)
            .fail(ajaxError);
    }

    function setTheSpecStock(resp) {
        var _quantity = resp.quantity;
        var _len = (_quantity < 10 ? _quantity : 10);
        var _str = "";
        $('#no-stock-hint').removeClass('shake');
        if (_quantity === 0) {
            $('.js-fast-buy-func-blk').hide();
            $('#blk-btn-notify,#no-stock-hint').removeClass('hide');
            $('#no-stock-hint').addClass('shake');
            $.wait(1000).then(function() {
                $('#no-stock-hint').removeClass('shake')
            })
        } else {
            for (var i = 0; i < _len; i++) {
                _str += '<option value="' + (i + 1) + '">' + (i + 1) + '</option>';
            }
            $fastBuyNumSelect.html(_str);
            $('.js-fast-buy-func-blk').show();
            $('#blk-btn-notify,#no-stock-hint').addClass('hide');
            $('#no-stock-hint').removeClass('shake');
        }
    ...
    function updateShoppingItemNum() {
        var _self = $(this);
        var _num = Math.floor(parseInt(_self.val(), 10));
        var _pid = _self.data('product-id');
        var _specId = _self.data('product-spec');
        $currentNumIpt = _self;
        ...
        $.ajax({
            ...
            .done(refreshCartInfomation)
            .fail(ajaxError);
    }

    function removeItemFromCart(productId, productSpecId) {
        $.ajax({
                url: '/Shopping/RemoveFromCart',
                data: {
                    __RequestVerificationToken: getSiteToken(),
                    productId: productId,
                    specificationId: productSpecId
                },
                type: 'POST',
                dataType: 'json'
            })
            .done(removeItemFromCartSuccess)
            .done(removeItemFromShoppintList(productId, productSpecId))
            .fail(ajaxError);
    }

    function removeItemFromShoppintList(pid, specId) {
        $('#shopping-item-' + pid + "-" + specId).slideUp(300, function() {
            $(this).remove();
        });
    }
    ...
    function removeItemFromCartSuccess(result) {
        if ($.isPlainObject(result)) {
            if (result.success === true) {
                ...
                refreshCartInfomation(result);
                getCartInformationSuccess(result);
            } else {
                swal({
                    title: result.msg,
                    text: "",
    ...
    function refreshCartInfomation(result) {
        if (result.success) {
            if ($currentNumIpt) {
                $currentNumIpt.data('buy-num', $currentNumIpt.val());
            }
            $('#shopping-items-num').text(result.quantity);
            $('#shopping-total').text(result.total);
            $('#shopping-payment').text(result.payment);

            $feeFrozen.toggleClass('hide', result.frozen == '0');
            $feeNormal.toggleClass('hide', result.normal == '0');
            $feeRefrigeration.toggleClass('hide', result.refrigeration == '0');
            $feeOther.toggleClass('hide', result.other == '0');
            if (result.frozen == "0" && result.normal == "0" && result.refrigeration == "0" && result.other == "0") {
                $shoppingFeeBlk.addClass("hide");
            } else {
                $shoppingFeeBlk.removeClass("hide");
            }
            if (result.item) {
                var _obj = result.item;
                $('#shopping-item-' + _obj.ProductID + "-" + _obj.SpecificationID).find('.total-price').find('.num').text(_obj.Total);
            }
        } else {
            swal({
                title: "oops",
            ...
            if ($currentNumIpt) {
                $currentNumIpt.val($currentNumIpt.data('buy-num'));
            }
        }
    }
    ...
    function removeConfirm() {
        ...
        swal({
            ...
            removeItemFromCart(_pid, _specId);
        });
    }

    function getCartInformation(result) {
        $.ajax({
                url: '/Shopping/CartInformation',
                data: {
                    __RequestVerificationToken: getSiteToken()
                },
                type: 'POST',
            ...
            .done(getCartInformationSuccess);
    }

    function getCartInformationSuccess(result) {
        var _num = parseInt(result.quantity, 10);
        if (_num == 0) return;
    ...
    function addItem2Cart() {
        ...
        var num = parseInt($cartNum.text(), 10);
        var quantity = $fastBuyNumSelect.val() || 1;
        var productId = _self.data('product-id');
        currentSpecId = $fastBuySpecSelect.val() || _self.data('product-spec');
        if (currentSpecId === '0') {
            swal({
                title: '請先選擇規格',
                text: "",
        ...
        $.ajax({
            ...
            .done(onAddItemSuccess)
            .fail(ajaxError)
    }

    function onAddItemSuccess(result) {
        if ($.isPlainObject(result)) {
            if (result.success === true) {
